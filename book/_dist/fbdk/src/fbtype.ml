...
let typecheck e =
  ...
  let rec derive_type assumptions expr =
    ...
    let (t, c) =
      ...
      | Let(i, e1, e2) ->
        let v = fresh_type_var () in
        let (t1, c1) = derive_type assumptions e1 in
        let (t2, c2) = derive_type (AssumptionMap.add i v assumptions) e2 in
        t2, constraint_union_of [c1; c2; (constraint_set_of [t1, v])]
      | LetRec(i1, i2, e1, e2) ->
  ...
  in
  let validate_map_acyclic m =
    let rec examine t visited =
      match t with
      | TInt -> ()
      | TBool -> ()
      | TVar(s) ->
        let i = Ident(s) in
        if IdentSet.mem i visited
        then
          (raise (TypeInferenceFailure ("type variable '"^s^" is defined in terms of itself")))
        else
          (try
             let t' = AssumptionMap.find i m in
             examine t' (IdentSet.add i visited)
           with
             Not_found -> ()
          )
      | TArrow (t1,t2) -> examine t1 visited; examine t2 visited
    in
    AssumptionMap.iter (fun i -> fun t -> examine t (IdentSet.singleton i)) m
  in
