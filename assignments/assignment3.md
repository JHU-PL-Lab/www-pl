...
1.  [7 points] Compute the following **substitutions**. Note that the substitution function is defined formally in Section 2.3.2 of the book.

    a.  `(Fun z -> Fun y -> x and True) [False/x]`

    b.  `(Fun x -> Fun z -> 5 + z) [6/z]`

    c.  `(Fun x -> Fun z -> If y Then x Else z) [True/y]`
