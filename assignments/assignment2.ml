...
(* Problem 2 *****************************************************************)
(* 
In the next few problems, you are more or less working with SIS. You will be
working with courses, course rosters, and waitlists. Each course is stored as
a record with the following type signature: 
*)
...
signups: A list of student names. The first n students in
the list (where n is the enrollment limit) are counted as being enrolled in the
course. Everyone else in the list are counted as being on the course's
waitlist.
...
These records can then stored in a list to form a list of courses, i.e. classes.
*)

(*

Complete the functions return_enrollment and return_waitlist.
...
let l3 = l1 @ l2;;
assert( return_enrollment l1 (EN(601,426)) = ["Kelvin Qian"; "Devin Hill"; "Fionn Connolly"] );;
assert( return_waitlist l1 (EN(601,426)) = [] );;
assert( try (return_enrollment l1 (EN(601,626))) with Not_found -> true );;
assert( try (return_waitlist l1 (EN(601,626))) with Not_found -> true );;
assert( return_enrollment l2 (EN(601,626)) = ["Yunmo Chen"; "Chang Liu"] );;
assert( return_waitlist l2 (EN(601,626)) = ["Shiwei Weng"; "Leandro Facchinetti"] );;
assert( try (return_enrollment l2 (EN(601,426))) with Not_found -> true );;
assert( try (return_waitlist l2 (EN(601,426))) with Not_found -> true );;
assert( return_enrollment l3 (EN(601,426)) = ["Kelvin Qian"; "Devin Hill"; "Fionn Connolly"] );;
assert( return_waitlist l3 (EN(601,426)) = [] );;
assert( return_enrollment l3 (EN(601,626)) = ["Yunmo Chen"; "Chang Liu"] );;
assert( return_waitlist l3 (EN(601,626)) = ["Shiwei Weng"; "Leandro Facchinetti"] );;

*)

(*

Write a function check_enrollment that, given a course list, a course number,
...
If the student is neither enrolled nor on the waitlist, check_enrollment should
return 
        NotRegistered
...
assert( check_enrollment l1 (EN(601,426)) "Fionn Connolly" = Enrolled );;
assert( check_enrollment l1 (EN(601,426)) "Yunmo Chen" = NotRegistered  );;
assert( check_enrollment l1 (EN(601,626)) "Yunmo Chen" = CourseNotFound );;
assert( check_enrollment l2 (EN(601,626)) "Chang Liu" = Enrolled );;
assert( check_enrollment l2 (EN(601,626)) "Shiwei Weng" = Waitlist(1,2) );;
assert( check_enrollment l2 (EN(601,626)) "Kelvin Qian" = NotRegistered );;
assert( check_enrollment l2 (EN(601,426)) "Kelvin Qian" = CourseNotFound );;
assert( check_enrollment l3 (EN(601,426)) "Devin Hill" = Enrolled );;
assert( check_enrollment l3 (EN(601,626)) "Leandro Facchinetti" = Waitlist(2,2) );;
...
*)
(*
5a. Use a ref variable to implement a counter that counts the number of times
...
(*
5b. Implement the function count_fib_immut that does the same thing as the
function count_fib_mut, but does not use immutability. (We will check this!)
...
(* Here is a data type which can be used to describe arithmetic
   expressions such as "2+3" or "4*2*7" or even "2*(8-3*9)".  Division is
   excluded so we can work just with integers.

   Note that the data type below is a tree, something like the
   binary tree data type we used during lecture. *)
...
(* This variable contains the arithmetic expression "-3 + (2 * 6)".  The
   parentheses aren't necessary in this text due to the way mathematical
   notation works, but we have to pick a shape for our tree and the parentheses
   help to emphasize that.  This expression says that we want to add the
   expression "-3" to the expression "2 * 6", so that's what we'll build. *)
let negated_three_plus_two_times_six = Plus(Negate(Integer(3)),Times(Integer(2),Integer(6)));;

(* This variable contains a more complex arithmetic expression:
   "(4 * (2 + 8)) - 3".  Again, the outermost partentheses aren't necessary in
   mathematical convention but the innermost parentheses are.  We don't need
   parentheses in our tree because the grouping that parentheses describe is
   handled instead by the parent-child relationship between nodes. *)
...
(* This variable contains the arithmetic expression "((4 * 2) + 8) - 3", which
   is equivalent to "4 * 2 + 8 - 3" by convention.  Note that both this
   expression and the previous expression performed those operations (e.g. you
   can add parentheses to the string "4 * 2 + 8 - 3" to get either one) but the
   grouping of the operations is reflected by the shape of the tree. *)
...
(* Write a function called "uses_times" which determines whether a given
   expression contains a multiplication operation. *)
let rec uses_times (e : expr) : bool = failwith "Not Implemented";;

(* Write a function called "get_all_integers" which produces a list of the
...
(* Write a recursive function which will transform an expression
   into a string.  (This process is sometimes called "pretty-printing".)  Be
   careful to handle parentheses correctly!  You are permitted to add
   parentheses to the string at every level of the expression but for fun
   try to minimize the number of parentheses that your function generates! *)

let rec pretty_print (e : expr) : string = failwith "Not Implemented";;

(* Write a recursive function to evaluate arithmetic expressions to integers.
...
(* Problem 5 *****************************************************************)
(* 
Port your arithmetic evaluator in Q3 to a module: make a file arith.ml which is a
standalone module implementation of the expr type, and the eval function
defined above (just copy your final code into that file, you are submitting two
versions of the same question answer).

We will test it by compiling and #load-ing your arith.ml into the top loop and invoking
