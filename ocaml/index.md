...
* We recommend you use the plugin called **OCaml Platform** for OCaml support in VSCode.   To install it, first run `opam install ocaml-lsp-server` from your shell.  Then from the `View` menu select `Extensions`, then type in OCaml in the search box and this extension will show up: select **OCaml Platform** from the list.

* (If you are having problems with installing OCaml Platform you could try to install the **OCaml and Reason IDE** extension instead (it is outdated however): from the `View` menu select `Extensions`, then type in OCaml in the search box and this extension will show up; install it.)
